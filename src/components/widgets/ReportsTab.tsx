...
function CapitalCard() {
  ...
  // Savings valuation at current sell rates for patrimony
  ...
  // Net Patrimony = Total Equity - Expenses + Savings (at current value)
...
function SavingsCard() {
  ...
  // Calculate current value at sell rates
  ...
  // Currencies available for savings
  const savingsCurrencies: Currency[] = ["USD", "EUR", "CAD"];

  // Available inventory for the selected currency
  ...
  return (
    <div className="bg-(--bg-primary) rounded-2xl p-5 border border-(--border-primary)">
      ...
      {/* Add Form — currency comes from portfolio */}
      <AnimatePresence>
        {showForm && (
          <motion.div
            initial={{ height: 0, opacity: 0 }}
          ...
          {savings.slice(0, 10).map((entry) => {
            ...
            return (
              ...
              >
                ...
                {/* Withdraw panel — editable amount */}
                <AnimatePresence>
                  {isWithdrawing && (
                    <motion.div
                      initial={{ height: 0, opacity: 0 }}
